...
// Сброс вида карты при смене вкладки
...
export default function MapView({ tab }) {
  ...
  const pins = tab === 'day' ? dayPins : congrats
  const icon = tab === 'day' ? iconRose : iconGold
  ...
  return (
    <div className="map-wrap">
      ...
      {tab === 'congrats' && congrats.length === 0 && !loading && (
      ...
      {selected && (
        <MediaModal pin={selected} tab={tab} onClose={() => setSelected(null)} />
      )}
    </div>
  )
}
